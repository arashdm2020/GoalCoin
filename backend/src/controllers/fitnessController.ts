...
const logWarmup = async (req: Request, res: Response) => {
  try {
    ...
    // Note: XP history is tracked via warmup_logs table (xp_earned field)
...
const logWorkout = async (req: Request, res: Response) => {
  try {
    ...
    // Note: XP history is tracked via workout_logs table (xp_earned field)
    // Note: Streak update is done in the raw SQL above to avoid schema mismatch issues
    // await updateUserStreak(userId);
...
const logMeal = async (req: Request, res: Response) => {
  try {
    ...
    // Note: XP history is tracked via meal_logs table (xp_earned field)
    // Note: Streak update is done in the raw SQL above to avoid schema mismatch issues
    // await updateUserStreak(userId);
